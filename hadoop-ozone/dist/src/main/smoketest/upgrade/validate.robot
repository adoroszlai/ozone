...
Read key created with Ozone Shell using S3 API
    ${output} =         Execute AWSS3APICli and checkrc    get-object --bucket ${PREFIX}-bucket --key key1-shell ${TEMPDIR}/get-result    0
                        Should contain    ${output}    "ContentLength"
    ${output} =         Execute and checkrc    cat ${TEMPDIR}/get-result    0
                        Should contain    ${output}    ${PREFIX}: another key created using Ozone Shell
                        Execute and checkrc    rm ${TEMPDIR}/get-result    0

Read key created with S3 API using S3 API
    ${output} =         Execute AWSS3APICli and checkrc    get-object --bucket ${PREFIX}-bucket --key key2-s3api ${TEMPDIR}/get-result    0
                        Should contain    ${output}    "ContentLength"
    ${output} =         Execute and checkrc    cat ${TEMPDIR}/get-result    0
